...
import { parseMonto, formatoInputMonto, numeroAFormatoInput, conectarInputMonto, toast, fechaHoyLarga } from './utils.js';
...
export function initCotizacion(){
  ...
  tipoDocToggle.querySelectorAll('button').forEach(btn => {
    if (btn.dataset.valor === estado.tipoDoc) btn.classList.add('activo'); else btn.classList.remove('activo');
    btn.addEventListener('click', () => {
      tipoDocToggle.querySelectorAll('button').forEach(b => b.classList.remove('activo'));
  ...
  estado.items.forEach((item, idx) => {
    ...
    el.innerHTML = `
      ...
      <div class="item-precio-row">
        ...
        <div class="pill-toggle" data-precio-toggle>
    ...
    el.querySelector('[data-accion="quitar"]').addEventListener('click', () => quitarItem(item.id));
    ...
    el.querySelectorAll('[data-precio-toggle] button').forEach(btn => {
      btn.addEventListener('click', () => {
        el.querySelectorAll('[data-precio-toggle] button').forEach(b => b.classList.remove('activo'));
        btn.classList.add('activo');
...
function generarDocumento(){
  ...
  html += `
    ...
    <p>
      ...
      Forma de pago: transferencia, efectivo, contado entregando el 30% financiás el restante en 30, 60, 90 días sin interés y hasta 360 días con interés, a través de Banco Nación hasta 36 meses (consultar tasas).
    </p>
